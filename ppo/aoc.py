...
def get_parse():
    ...
    parser.add_argument("--exp-name", type=str, default="aoc",
...
def run(args):
    ...
    key, actor_key, oracle_critic_key = jax.random.split(key, 3)
    ...
    def get_action_and_value_for_sample(
        ...
        return action, logprob, oracle_value.squeeze(1), key
    ...
    def get_action_and_value_for_train(
        ...
        return logprob, entropy, oracle_value
    ...
    def step_once(carry, step, env_step_fn):
        ...
        action, logprob, oracle_value, key = get_action_and_value_for_sample(agent_state.params, obs, key)
    ...
    def compute_gae(
        ...
        _, advantages = jax.lax.scan(
            compute_gae_once, advantages, (dones[1:], oracle_values[1:], oracle_values[:-1], storage.rewards), reverse=True
        )
        storage = storage.replace(
            advantages=advantages,
            oracle_returns=advantages + storage.oracle_values,
        )
        return storage

    def ppo_loss(params, x, a, logp, mb_advantages, mb_oracle_returns):
        newlogprob, entropy, oracle_newvalue = get_action_and_value_for_train(params, x, a)
        ...
        loss = pg_loss - args.ent_coef * entropy_loss + oracle_v_loss * args.vf_coef
        return loss, (pg_loss, oracle_v_loss, entropy_loss, jax.lax.stop_gradient(approx_kl))
    ...
    def update_ppo(
        ...
        def update_epoch(carry, unused_inp):
            ...
            def update_minibatch(agent_state, minibatch):
                (loss, (pg_loss, oracle_v_loss, entropy_loss, approx_kl)), grads = ppo_loss_grad_fn(
                    agent_state.params,
                    minibatch.obs,
                    minibatch.actions,
                    minibatch.logprobs,
                ...
                return agent_state, (loss, pg_loss, oracle_v_loss, entropy_loss, approx_kl, grads)

            agent_state, (loss, pg_loss, oracle_v_loss, entropy_loss, approx_kl, grads) = jax.lax.scan(
                update_minibatch, agent_state, shuffled_storage
            )
            return (agent_state, key), (loss, pg_loss, oracle_v_loss, entropy_loss, approx_kl, grads)

        (agent_state, key), (loss, pg_loss, oracle_v_loss, entropy_loss, approx_kl, grads) = jax.lax.scan(
            update_epoch, (agent_state, key), (), length=args.update_epochs
        )
        return agent_state, loss, pg_loss, oracle_v_loss, entropy_loss, approx_kl, key
    ...
    for update in tbar:
        ...
        agent_state, loss, pg_loss, oracle_v_loss, entropy_loss, approx_kl, key = update_ppo(
            agent_state,
            storage,
            key,
        )

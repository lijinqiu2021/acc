...
def get_parse():
    ...
    parser.add_argument("--exp-name", type=str, default="dropout",
    ...
    parser.add_argument("--dropout-rate", type=float, default=1 / 2, choices=[1 / 2, 2 / 3],
                        help="proportion of the learning stage to dropout oracle observation in Suphx")
...
def run(args):
    ...
    sample_obs = np.concatenate([sample_obs, sample_obs], axis=0)
    sample_obs = sample_obs[np.newaxis, ...]
    agent_state = TrainState.create(
        apply_fn=None,
        params=AgentParams(
            actor.init(actor_key, sample_obs),
            critic.init(critic_key, sample_obs),
        ),
        tx=optax.chain(
    ...
    def get_action_and_value_for_sample(
        ...
        logits = actor.apply(params.actor_params, obs)
    ...
    def get_action_and_value_for_train(
        ...
        value = critic.apply(params.critic_params, obs).squeeze()
        return logprob, entropy, value

    def mask_observation(key, obs, dones, mask_maps, dropout_prob, mask_prob):
        """mask obs, concat the executor_obs and dropouted oracle_obs"""
        ...
        key, sub_key = jax.random.split(key, 2)
        oracle_left_mask = jax.random.choice(
            key=sub_key, a=jnp.array([0, 1]), shape=obs.shape, p=jnp.array([dropout_prob, (1 - dropout_prob)])
        )
        dropout_obs = jnp.concatenate([executor_obs, obs * oracle_left_mask], axis=1)
        return key, dropout_obs, mask_maps
    ...
    def step_env_wrappeed(episode_stats, handle, action, key, mask_maps, dropout_prob):
        handle, (next_obs, reward, next_done, info) = step_env(handle, action)
        key, next_obs, mask_maps = mask_observation(key, next_obs, next_done, mask_maps, dropout_prob)
    ...
    def step_once(carry, step, env_step_fn):
        agent_state, episode_stats, obs, done, key, handle, mask_maps, dropout_prob = carry
        action, logprob, value, key = get_action_and_value_for_sample(agent_state.params, obs, key)

        episode_stats, handle, (next_obs, reward, next_done, _), key, mask_maps = env_step_fn(
            episode_stats, handle, action, key, mask_maps, dropout_prob
        )
        storage = Storage(
            obs=obs,
            actions=action,
        ...
        return ((agent_state, episode_stats, next_obs, next_done, key, handle, mask_maps, dropout_prob), storage)

    def rollout(
        agent_state, episode_stats, next_obs, next_done, key, handle, mask_maps, dropout_prob, step_once_fn, max_steps
    ):
        (agent_state, episode_stats, next_obs, next_done, key, handle, mask_maps, dropout_prob), storage = jax.lax.scan(
            step_once_fn,
            (agent_state, episode_stats, next_obs, next_done, key, handle, mask_maps, dropout_prob),
            (),
            max_steps,
        )
    ...
    def ppo_loss(params, x, a, logp, mb_advantages, mb_returns):
        newlogprob, entropy, newvalue = get_action_and_value_for_train(params, x, a)
    ...
    # start the game
    global_step = 0
    dropout_prob = min(global_step / (args.total_timesteps * args.dropout_rate), 1)
    start_time = time.time()
    next_obs = envs.reset()
    next_done = jnp.zeros(args.num_envs, dtype=jax.numpy.bool_)
    key, next_obs, mask_maps = mask_observation(
        key, next_obs, jnp.ones(args.num_envs, dtype=jax.numpy.bool_), next_obs, dropout_prob
    )
    ...
    for update in tbar:
        ...
        writer.add_scalar("train/dropout_prob", dropout_prob, global_step)
